...
@section('page-title', 'Resubmit Request')
...
<div class="d-flex align-items-center justify-content-between mb-4 flex-wrap gap-2">
    <div>
        <h1 style="font-family:'Syne',sans-serif;font-weight:800;font-size:1.6rem;color:var(--navy);margin:0;">
            {{ app()->getLocale() === 'ar' ? 'إعادة إرسال الطلب' : 'Resubmit Request' }}
        </h1>
        <p style="color:var(--muted);font-size:0.875rem;margin-top:4px;">
            {{ app()->getLocale() === 'ar' ? 'أجرِ أي تغييرات أدناه وأرسل طلباً جديداً بناءً على #' . $citizenRequest->id . '.' : 'Make any changes below and submit a new request based on #' . $citizenRequest->id . '.' }}
        </p>
    </div>
...
{{-- Original rejection banner --}}
<div style="background:rgba(239,68,68,0.08);border:1px solid rgba(239,68,68,0.25);border-radius:10px;padding:0.875rem 1.25rem;margin-bottom:1.5rem;font-size:0.875rem;color:#991b1b;display:flex;align-items:flex-start;gap:0.75rem;">
    <i class="bi bi-x-circle-fill flex-shrink-0" style="margin-top:2px;"></i>
    <div>
        @php $ar = app()->getLocale() === 'ar'; @endphp
        <strong>{{ $ar ? 'الطلب الأصلي #' : 'Original request #' }}{{ $citizenRequest->id }}</strong> {{ $ar ? 'تم رفضه.' : 'was declined.' }}
        @if($citizenRequest->histories->where('to_status','rejected')->last()?->note)
            <div style="color:var(--muted);margin-top:4px;">
                {{ $ar ? 'ملاحظة الإدارة:' : 'Admin note:' }} "{{ $citizenRequest->histories->where('to_status','rejected')->last()->note }}"
            </div>
        @endif
    </div>
</div>
...
<div class="col-lg-7">
    <div class="app-card">
        <form method="POST" action="{{ route('citizen.requests.resubmit.store', $citizenRequest) }}" enctype="multipart/form-data">
            @csrf
