...
@auth
<script>
// ── Global Chat Message Popup ──────────────────────────────────────────────
(function () {
    const POLL_URL = '{{ Auth::user()->role === "citizen" ? route("messages.poll") : route("office.messages.poll") }}';
    ...
    function pollMessages() {
        ...
        .then(messages => {
            ...
            // Show one toast per new message (max 3 to avoid spam)
    ...
    // Init: on first load set lastId to current max so we don't spam old messages
    if (lastId === 0) {
